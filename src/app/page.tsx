...
// --- TYPE DEFINITIONS ---
// Kept in the same file to reduce file count
...
function ThemeSelector({ currentTheme, onThemeChange }: { currentTheme: ThemeName, onThemeChange: (theme: ThemeName) => void }) {
  return (
...
function InvoiceHistory({ invoices, onLoad, onDelete, activeId, searchTerm, setSearchTerm }: { invoices: Invoice[], onLoad: (id: string) => void, onDelete: (id: string) => void, activeId: string, searchTerm: string, setSearchTerm: (term: string) => void }) {
...
function SettingsPage({ settings, setSettings }: { settings: AppSettings, setSettings: React.Dispatch<React.SetStateAction<AppSettings>> }) {
...
function InvoiceForm({ invoice, setInvoice }: { invoice: Invoice, setInvoice: React.Dispatch<React.SetStateAction<Invoice>> }) {
...
function InvoicePage({ onNewInvoice, activeInvoice, setActiveInvoice, history, setHistory }: { onNewInvoice: () => void; activeInvoice: Invoice | null; setActiveInvoice: React.Dispatch<React.SetStateAction<Invoice | null>>; history: Invoice[]; setHistory: React.Dispatch<React.SetStateAction<Invoice[]>> }) {
  const { toast } = useToast();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <div className="lg:col-span-3">
          <div id="invoice-preview-wrapper" className="shadow-lg rounded-lg border bg-card text-card-foreground">
            <div ref={previewRef} id="invoice-preview-container"><InvoicePreview invoice={activeInvoice} /></div>
          </div>
        </div>
      </div>
    </>
      ...
      <div className="flex min-h-screen">
        ...
        <SidebarInset>
          <div className="p-4 sm:p-6 lg:p-8 flex flex-col h-full">
            ...
            {activeView === 'invoice' && <InvoicePage onNewInvoice={createNewInvoice} activeInvoice={activeInvoice} setActiveInvoice={setActiveInvoice} history={history} setHistory={setHistory} />}
